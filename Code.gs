...
var CONFIG = {
  ...
  TIERS: {
    MEMBER:   { name: 'Elite Access',   min: 0    },
    GOLD:     { name: 'Elite Preferred',     min: 1000 },
    PLATINUM: { name: 'Elite Prime', min: 3000 },
    DIAMOND:  { name: 'Elite Prestige',  min: 5000 },
  },
};
...
function serveEnroll() {
  ...
  html.setTitle('Join Urbana Elite');
...
function serveAdmin() {
  ...
  html.setTitle('Urbana Elite — Admin');
  html.setXFrameOptionsMode(HtmlService.XFrameOptionsMode.ALLOWALL);
  return html;
}
...
function getTier(pts) {
  if (pts >= 5000) return 'ELITE_PRESTIGE';
  if (pts >= 3000) return 'ELITE_PRIME';
  if (pts >= 1000) return 'ELITE_PREFERRED';
  return 'ELITE_ACCESS';
}

function getNextTier(pts) {
  if (pts >= 5000) return { name: 'Elite Prestige',  needed: 0,          at: 5000 };
  if (pts >= 3000) return { name: 'Elite Prestige',  needed: 5000 - pts, at: 5000 };
  if (pts >= 1000) return { name: 'Elite Prime', needed: 3000 - pts, at: 3000 };
  return                  { name: 'Elite Preferred',     needed: 1000 - pts, at: 1000 };
}
...
function enrollMember(p) {
  ...
  sheet.appendRow([memberId, firstName, lastName, email, phone,
    str(p.nationality), str(p.dob), joinDate, 0, 'ELITE_ACCESS', mktg, 'ACTIVE', '', '']);

  try { sendWelcomeEmail(email, firstName, memberId); } catch(e) {}

  return { success: true, memberId: memberId,
    message: 'Welcome to Urbana Elite! Your Member ID is ' + memberId };
}
...
function getDashboard(p) {
  ...
  for (var i = 1; i < mData.length; i++) {
    ...
    var t = str(mData[i][9]);
    if (t==='ELITE_PREFERRED') gold++; else if (t==='ELITE_PRIME') platinum++; else if (t==='ELITE_PRESTIGE') diamond++;
    try {
...
function sendOTPEmail(email, firstName, otp) {
  GmailApp.sendEmail(email,
    'Your Urbana Elite login code: ' + otp,
...
function sendTierUpgradeEmail(email, firstName, newTier) {
  ...
  GmailApp.sendEmail(email,
    ...
    'Dear ' + firstName + ',\n\nYou are now an Urbana Elite ' + n + ' member.\n\n'
    + 'Warm regards,\nUrbana Langsuan Hotel & Residence');
}
